...
class TestJWTUtils(unittest.TestCase):
    ...
    def test_encode_access_token(self):
        ...
        with self.app.test_request_context():
            ...
            self.assertEquals(data['identity'], identity)
            self.assertEquals(data['fresh'], True)
            self.assertEquals(data['type'], 'access')
            self.assertEquals(data['user_claims'], user_claims)
            self.assertEquals(data['iat'], data['nbf'])
            ...
            self.assertIn('user_claims', data)
            self.assertEquals(data['identity'], identity)
            self.assertEquals(data['fresh'], False)
            self.assertEquals(data['type'], 'access')
            self.assertEquals(data['user_claims'], user_claims)
            self.assertEquals(data['iat'], data['nbf'])
            now_ts = calendar.timegm(datetime.utcnow().utctimetuple())
    ...
    def test_encode_refresh_token(self):
        ...
        with self.app.test_request_context():
            ...
            self.assertIn('identity', data)
            self.assertEquals(data['identity'], identity)
            self.assertEquals(data['type'], 'refresh')
            self.assertEquals(data['iat'], data['nbf'])
            now_ts = calendar.timegm(datetime.utcnow().utctimetuple())
            exp_seconds = data['exp'] - now_ts
            self.assertLessEqual(exp_seconds, 60 * 5)
